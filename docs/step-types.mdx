...
Native targets reject non-empty hunt-level `assertions:` blocks before launch. Use inline `assert: visible` and `assert: notVisible` steps for native UI checks.
