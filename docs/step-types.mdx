...
:::note
These steps target the **web**. On the experimental [macOS target](/macos-target), a portable subset is supported on both web and macOS, although steps such as `screenshot`, `hover`, and `press` have target-specific behavior. Web-only steps (`navigate`, network mocking, dialogs, etc.) are rejected up front — see its step-compatibility matrix.
:::
