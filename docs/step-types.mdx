...
:::note
These steps target the **web**. On the experimental [macOS target](/macos-target), a portable subset runs unchanged while web-only steps (`navigate`, network mocking, dialogs, etc.) are rejected up front — see its step-compatibility matrix.
:::
