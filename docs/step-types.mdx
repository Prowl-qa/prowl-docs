...
The **web** target runs every step. On a **native** target, web-only steps are **rejected up front** at validation time — before anything launches — with a clear, target-labelled error; the same check runs for sub-hunts invoked via `runHunt`.
...
| `hover`, `scrollTo` | ✅ | ✅ | ❌ (no touch equivalent yet) |
...
`hover` and `scrollTo` are portable on macOS but **rejected on the touch targets** (Android and iOS) — they have no touch equivalent yet. A web-only step produces an error naming the offending step, for example:
...
See the [Android](/android) and [iOS](/ios) target pages for per-target selector dialects and step notes (e.g. `press` supports a smaller key set on iOS).
