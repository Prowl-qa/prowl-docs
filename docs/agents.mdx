...
Jump to the [Library API](#library-api-nodejs) for programmatic Node.js integration, the [MCP Server](/mcp) to connect an agent without shell access, or [Using Starter Templates](#using-starter-templates) for hunt templates bundled with the CLI.
...
:::tip
That's the full loop — install, run, branch. The sections below cover the details: workflow patterns, the Node.js library API, CI integration, and starter templates.
:::
...
Prowl bundles a catalog of starter hunts — login flows, signup, password reset, checkout,
CRUD cycles, onboarding wizards, smoke tests, macOS apps — inside the npm package. An agent
should check the catalog before authoring a hunt from scratch:

```bash
prowl templates list --json              # discover: [{ id, category, name, description, tags }]
prowl templates show auth/login-flow     # inspect the YAML
prowl init --template auth/login-flow    # install into .prowl/hunts/login-flow.yml
prowl run login-flow --json              # execute
```

`prowl init --template` works on an already-initialized project (it only adds hunts) and
resolves every id before writing anything. See [Starter Templates](/starter-templates) for the
full category list and contribution rules.

### Library API for Templates

The same catalog is exposed by the library, alongside the functions for customizing and
running a hunt once it is installed:

```typescript
import {
  listTemplates, readTemplate, resolveTemplate,
  loadConfig, loadHunt, interpolateHunt, runHunt
} from "prowl-tools";

// Discover
const authTemplates = listTemplates().filter((t) => t.category === "auth");
const info = resolveTemplate("auth/login-flow");   // null if the id is unknown
const yaml = readTemplate("auth/login-flow");      // raw template text

// After `prowl init --template auth/login-flow`:
...
<div className="card-grid">
  <a className="card" href="/starter-templates">
    ...
    <p>Bundled hunts for common flows — browse, install, run</p>
  </a>
